...
| R082 (Phase 3+) | Mode flag + arq_control_plane token + entitlement protocol | TC-SIG-081..092 (planned) | PLANNED | BEHAVIORAL | See R083 below for the design. Phase 4 (collector profiles, entitlement metadata, status callbacks) is out of scope for R083 and remains future work. |
| R083 | Mode B authentication + configuration | TC-SIG-081..092 (planned) | PLANNED | BEHAVIORAL | Design-only rule. Adds `signals.mode: standalone \| arq_managed`, separate `arq_control_plane_token_file`/`_env`. Auth middleware compares against both tokens; `actor` derived from match. `arq_control_plane` actor only valid in `mode=arq_managed`. Startup-validation hard errors for missing/duplicate/short token, conflicting `_file`+`_env`. Implementation lands in a single follow-up slice. |
